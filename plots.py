...
class Plots:
    def imgs_dataset(data_dir='./image_dataset', classes=['regular', 'pneumonia', 'covid'], num_imgs=4, img_width=224, img_height=224):
        item_files = []
    ...
    def confusion_matrix_plot(all_cms):
        ...
        df_cm = pd.DataFrame(data_abs, index = [i for i in ["COVID-19", "Neumonía", "Saludable"]],
                          columns = [i for i in ["COVID-19", "Neumonía", "Saludable"]])
        
        sn.set(font_scale=1.5)

        # plt.xticks(np.arange(3)+0.5,("COVID-19", "Pneumonia", "Normal"), rotation=0, fontsize="17", va="center")
        plt.yticks(np.arange(3)+0.5,("COVID-19", "Neumonía", "Saludable"), rotation=0, fontsize="17", va="center")
        ...
        df_cm = pd.DataFrame(data_prec, index = [i for i in ["COVID-19", "Neumonía", "Saludable"]],
                  columns = [i for i in ["COVID-19", "Neumonía", "Saludable"]])
        sn.set(font_scale=1.5)
        ax.xaxis.tick_top()
        ...
        df_cm = pd.DataFrame(data_sens, index = [i for i in ["COVID-19", "Neumonía", "Saludable"]],
                          columns = [i for i in ["COVID-19", "Neumonía", "Saludable"]])
        # sn.set(font_scale=1.5)
